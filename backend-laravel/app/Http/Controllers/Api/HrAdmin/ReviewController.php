...
class ReviewController extends Controller
{
    ...
    public function applications(Request $request)
    {
        ...
        $payload = $applications->map(function ($row) use ($submissionCounts, $fallbackScores) {
            $faculty = [
                ...
                'last_promotion_date' => $row->last_promotion_date ?? $row->date_of_last_promotion,
    ...
    public function upsertAreaScore(Request $request)
    {
        ...
        // Ensure there is at least one criterion score row for this submission so
        // frontend modals that expect per-criterion data don't show empty rows when
        // an area total exists. We create/update a fallback criterion keyed by part_id
        // when available, otherwise by file name or submission id.
        try {
            $now = now();
            ...
            if (!empty($partId)) {
                $criterionKey = 'part:' . $partId;
                $criterionLabel = $partId;
            } elseif (!empty($filePath)) {
                $fileName = basename($filePath);
                $criterionKey = 'file:' . $fileName;
                $criterionLabel = $fileName;
            } else {
                $criterionKey = 'submission:' . $submissionId;
            ...
            $criterionRow = [
                ...
                'criterion_title' => 'Auto-created fallback criterion',
                'criterion_max_points' => (float) ($payload['hr_points'] ?? 0),
                'score' => (float) ($payload['hr_points'] ?? 0),
                'capped_score' => (float) ($payload['hr_points'] ?? 0),
                'excess_score' => 0,
                'reviewed_at' => $now,
                'reviewed_by' => null,
                'notes' => 'Auto-generated by upsertAreaScore to ensure per-criterion rows exist',
                'updated_at' => $now,
                'created_at' => $now,
            ];
            ...
            // Swallow errors here to avoid breaking the primary upsert flow; log for debugging.
            logger()->warning('Failed to ensure criterion row for submission ' . $submissionId . ': ' . $e->getMessage());
        }
    ...
    public function templateFile(Request $request, int $templateId)
    {
        $template = DB::table('area_part_templates')->where('template_id', $templateId)->first();

        if (!$template) {
            return response()->json(['error' => 'Template not found.'], 404);
        }

        $bucket = $template->storage_bucket ?: 'public';
        $storagePath = ltrim((string) $template->storage_path, '/');

        if (!Storage::disk($bucket)->exists($storagePath)) {
            return response()->json(['error' => 'Template file not found.'], 404);
        }

        $downloadName = $template->file_name ?: basename($storagePath);
        $mimeType = $template->mime_type ?: Storage::disk($bucket)->mimeType($storagePath);

        return Storage::disk($bucket)->response($storagePath, $downloadName, [
    ...
    public function uploadTemplate(Request $request)
    {
        ...
        $template = DB::table('area_part_templates')
        ...
        return response()->json([
            ...
            'storage_path' => $storagePath,
            'url' => $this->publicStorageUrl($request, $storagePath),
            'file_name' => $file->getClientOriginalName(),
        ]);
    }
