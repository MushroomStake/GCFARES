...
class EncryptPayload
{
    ...
    {
        $this->key = (string) (config('app.api_encryption_key') ?: env('API_ENCRYPTION_KEY'));
    }
    ...
    {
        $isTemplateUpload = $request->is('api/review/templates/upload');

        // 1. INCOMING REQUEST: Intercept and Decrypt incoming JSON from Frontend
        if (! $isTemplateUpload && ($request->isMethod('post') || $request->isMethod('put') || $request->isMethod('patch'))) {
